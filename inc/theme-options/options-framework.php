...
function of_get_option( $name, $default = false ) {
	//去数据库获取'optionsframework'的值，该句的作用是检测options-framework框架是否成功安装
	$config = get_option( 'optionsframework' );
	//var_dump($config);
	//die;//array (size=1) 'id' => string 'dayu' (length=4)
	//如果没有id就返回第二个参数的值$default
	if ( ! isset( $config['id'] ) ) {
		return $default;
	}
	//获取该主题下自定义的键值对，该句的作用可以使每个主题使用自己自定义的键值对
